...
import { CreateRecipeDto } from './recipes.validation';
...
export class RecipesRepository {
   ...
   */
  async create(data: CreateRecipeDto, userId: number, groupId: number | null) {
    const pool = await getPool();
    const result = await pool
      .request()
      ...
      .input('thoiGian', sql.Int, data.thoiGian || null)
      .input('khauPhan', sql.Int, data.khauPhan || null)
      .input('doKho', sql.NVarChar, data.doKho || 'Dễ')
      .input('danhMuc', sql.NVarChar, data.danhMuc || 'Khác')
      .input('moTa', sql.NVarChar, data.moTa || null)
      .input('hinhAnh', sql.NVarChar, data.hinhAnh || null)
      .input('maNhom', sql.Int, groupId)
      .input('maNguoiTao', sql.Int, userId)
      .query(`
        INSERT INTO MonAn (
      ...
      `);
    return result.recordset[0].MaMon as number;
  }
   ...
   */
  async update(id: number, data: Partial<CreateRecipeDto>) {
    const pool = await getPool();
    await pool
      .request()
      .input('id', sql.Int, id)
      ...
      .input('thoiGian', sql.Int, data.thoiGian || null)
      .input('khauPhan', sql.Int, data.khauPhan || null)
      .input('doKho', sql.NVarChar, data.doKho || null)
      .input('danhMuc', sql.NVarChar, data.danhMuc || null)
      .input('moTa', sql.NVarChar, data.moTa || null)
      .input('hinhAnh', sql.NVarChar, data.hinhAnh || null)
      .query(`
        UPDATE MonAn SET
   ...
   */
  async getSuggestedByInventory(groupId: number) {
    const pool = await getPool();
    const result = await pool
      .request()
   ...
   */
  async deductInventoryForCooking(
    monId: number,
    groupId: number,
    multiplier: number
