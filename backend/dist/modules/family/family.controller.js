...
const svc = new family_service_1.FamilyService();
class FamilyController {
    // GET /api/v1/family/me — danh sách nhóm của tôi
    async getMyFamilies(req, res, next) {
        try {
    ...
    async create(req, res, next) {
        try {
            ...
            return (0, response_1.createSuccess)(res, data, 'Tạo nhóm thành công', 201);
        }
        catch (e) {
            next(e);
        }
    ...
    async generateInvite(req, res, next) {
        try {
            const groupId = Number(req.params.groupId);
            const dto = { maxUses: req.body.maxUses };
            const data = await svc.generateInvite(req.user.id, groupId, dto);
            return (0, response_1.createSuccess)(res, data, 'Tạo mã mời thành công', 201);
        }
        catch (e) {
            next(e);
        }
    ...
    async joinFamily(req, res, next) {
        try {
            ...
            return (0, response_1.createSuccess)(res, data, `Tham gia nhóm "${data.TenNhom}" thành công`);
        }
        catch (e) {
            next(e);
        }
    ...
    async removeMember(req, res, next) {
        try {
            ...
            return (0, response_1.createSuccess)(res, null, 'Đã xóa thành viên khỏi nhóm');
        }
        catch (e) {
            next(e);
        }
    ...
    async revokeInvite(req, res, next) {
        try {
            ...
            return (0, response_1.createSuccess)(res, null, 'Đã thu hồi mã mời');
        }
        catch (e) {
            next(e);
        }
    ...
    async transferLeadership(req, res, next) {
        try {
            ...
            if (!newLeaderId)
                throw { statusCode: 400, message: 'Thiếu thông tin người nhận chuyển nhượng' };
            await svc.transferGroupLeadership(groupId, Number(newLeaderId), req.user.id);
            return (0, response_1.createSuccess)(res, null, 'Chuyển nhượng quyền chủ hộ gia đình thành công');
        }
        catch (e) {
            next(e);
        }
    ...
    async updateFamilyInfo(req, res, next) {
        try {
            const groupId = Number(req.params.groupId);
            const { name, description, lastSeenUpdatedAt } = req.body;
            if (!name)
                throw { statusCode: 400, message: 'Tên nhóm gia đình không được để trống' };
            if (!lastSeenUpdatedAt)
                throw { statusCode: 400, message: 'Thiếu mã thời gian cập nhật gần nhất' };
            const data = await svc.updateGroupInfo(groupId, name, description, lastSeenUpdatedAt, req.user.id);
            return (0, response_1.createSuccess)(res, data, 'Cập nhật thông tin gia đình thành công');
        }
        catch (e) {
            next(e);
        }
