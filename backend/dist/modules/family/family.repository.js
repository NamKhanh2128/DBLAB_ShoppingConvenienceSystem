...
class FamilyRepository {
    ...
    async createGroup(leaderId, dto) {
        ...
        const result = await pool.request()
            .input('name', mssql_1.default.NVarChar(100), dto.name)
            .input('desc', mssql_1.default.NVarChar(500), dto.description ?? null)
            .input('maxMembers', mssql_1.default.Int, dto.maxMembers ?? 10)
            .input('leader', mssql_1.default.Int, leaderId)
            .query(`
      ...
      `);
        ...
        await pool.request()
            ...
            .input('u', mssql_1.default.Int, leaderId)
    ...
    async addMember(groupId, userId) {
        ...
        await pool.request()
            .input('g', mssql_1.default.Int, groupId)
            .input('u', mssql_1.default.Int, userId)
    ...
    // ── INVITE ─────────────────────────────────────────────────────────────────
    async createInvite(groupId, creatorId, code, maxUses, expiresAt) {
        const pool = await (0, database_1.getPool)();
        const result = await pool.request()
            .input('groupId', mssql_1.default.Int, groupId)
            .input('creatorId', mssql_1.default.Int, creatorId)
            .input('code', mssql_1.default.NVarChar(8), code)
            .input('maxUses', mssql_1.default.Int, maxUses)
    ...
    async getInvitesByGroup(groupId) {
        const pool = await (0, database_1.getPool)();
        const result = await pool.request()
            .input('groupId', mssql_1.default.Int, groupId)
            .query(`
    ...
    async revokeInvite(inviteId, requesterId) {
        const pool = await (0, database_1.getPool)();
        await pool.request()
            .input('id', mssql_1.default.UniqueIdentifier, inviteId)
            .input('requesterId', mssql_1.default.Int, requesterId)
            .query(`
     ...
     */
    async joinFamilyTransaction(code, userId) {
        const pool = await (0, database_1.getPool)();
        const tx = new mssql_1.default.Transaction(pool);
        try {
            await tx.begin();
            ...
            const inviteRes = await tx.request()
                .input('code', mssql_1.default.NVarChar(8), code.toUpperCase())
                .query(`
        ...
        `);
            ...
            if (!invite)
                throw { statusCode: 404, message: 'Mã mời không tồn tại' };
            if (invite.IsDeleted)
                throw { statusCode: 410, message: 'Mã mời đã bị thu hồi' };
            if (invite.NhomDeleted)
                throw { statusCode: 410, message: 'Nhóm không còn tồn tại' };
            if (new Date(invite.ExpiresAt) < new Date())
                throw { statusCode: 410, message: 'Mã mời đã hết hạn (48h)' };
            if (invite.UsedCount >= invite.MaxUses)
                throw { statusCode: 409, message: `Mã mời đã đạt giới hạn ${invite.MaxUses} lần dùng` };
            // BƯỚC 3: Kiểm tra xem user có đang thuộc nhóm nào khác trong hệ thống không
            const userGroupCheck = await tx.request()
                .input('userIdForGroupCheck', mssql_1.default.Int, userId)
            ...
            const countRes = await tx.request()
                .input('groupId2', mssql_1.default.Int, invite.MaNhom)
            ...
            await tx.request()
                .input('groupId3', mssql_1.default.Int, invite.MaNhom)
                .input('userId3', mssql_1.default.Int, userId)
            ...
            await tx.request()
                .input('code2', mssql_1.default.NVarChar(8), code.toUpperCase())
    ...
    // ─── LEADERSHIP TRANSFER ───────────────────────────────────────────────────
    async transferLeadershipTransaction(groupId, oldLeaderId, newLeaderId, oldLeaderName, newLeaderName) {
        const pool = await (0, database_1.getPool)();
        const tx = new mssql_1.default.Transaction(pool);
        try {
            await tx.begin();
            ...
            await tx.request()
                .input('groupId', mssql_1.default.Int, groupId)
                .input('newLeaderId', mssql_1.default.Int, newLeaderId)
            ...
            await tx.request()
                .input('groupId', mssql_1.default.Int, groupId)
                .input('oldLeaderId', mssql_1.default.Int, oldLeaderId)
            ...
            await tx.request()
                .input('groupId', mssql_1.default.Int, groupId)
                .input('content', mssql_1.default.NVarChar(500), logContent)
    ...
    async updateGroupInfoOCC(groupId, name, desc, lastSeenUpdatedAt) {
        const pool = await (0, database_1.getPool)();
        const result = await pool.request()
            .input('groupId', mssql_1.default.Int, groupId)
            .input('name', mssql_1.default.NVarChar(100), name)
            .input('desc', mssql_1.default.NVarChar(500), desc)
            .input('lastSeen', mssql_1.default.DateTime2, new Date(lastSeenUpdatedAt))
            .query(`
        UPDATE NhomGiaDinh 
    ...
    async logActivity(groupId, content, type) {
        const pool = await (0, database_1.getPool)();
        await pool.request()
            .input('groupId', mssql_1.default.Int, groupId)
            .input('content', mssql_1.default.NVarChar(500), content)
            .input('type', mssql_1.default.NVarChar(50), type)
