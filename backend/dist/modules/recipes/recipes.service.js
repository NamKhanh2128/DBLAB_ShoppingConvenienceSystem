...
class RecipesService {
    repo = new recipes_repository_1.RecipesRepository();
    ...
    // -------------------------------------------------------
    async getById(id, userId, groupId) {
    ...
    async create(data, userId, groupId) {
        ...
        const validatedData = recipes_validation_1.createRecipeSchema.parse(data);
    ...
    async update(id, data, userId) {
        ...
        await this.checkGroupMembership(recipe.MaNhom, userId);
        const validatedData = recipes_validation_1.createRecipeSchema.partial().parse(data);
    ...
    // -------------------------------------------------------
    async cookRecipe(id, data, userId) {
        // Validate input
        const validated = recipes_validation_1.cookRecipeSchema.parse(data);
        ...
        const result = await this.repo.deductInventoryForCooking(id, validated.maNhom, multiplier);
        return {
            message: `Đã trừ ${result.deducted} nguyên liệu khỏi kho`,
            deductedCount: result.deducted,
            notFoundIngredients: result.notFound,
            warning: result.notFound.length > 0
