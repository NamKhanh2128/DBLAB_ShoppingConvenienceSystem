...
const shopping_repository_1 = require("./shopping.repository");
const family_repository_1 = require("../family/family.repository");
const shopping_validation_1 = require("./shopping.validation");
class ShoppingService {
    repo = new shopping_repository_1.ShoppingRepository();
    ...
    async createList(data, userId) {
        const validated = shopping_validation_1.createListSchema.parse(data);
    ...
    async updateStatus(listId, status, userId) {
    ...
    async addItem(listId, data, userId) {
        ...
        const validated = shopping_validation_1.addItemSchema.parse(data);
        // Auto-merge: kiểm tra có item trùng tên+đơn vị không
        const existing = await this.repo.findDuplicateItem(listId, validated.tenThucPham, validated.donVi || null);
        if (existing) {
    ...
    async toggleItem(itemId, done, userId) {
        const item = await this.repo.getItemById(itemId);
        if (!item)
            throw { statusCode: 404, message: 'Không tìm thấy món hàng' };
    ...
    async updateItem(itemId, data, userId) {
        ...
        if (!item)
            throw { statusCode: 404, message: 'Không tìm thấy món hàng' };
        await this.checkListOwnership(item.MaDanhSach, userId);
        const validated = shopping_validation_1.updateItemSchema.parse(data);
     ...
     */
    async completeAndRestock(listId, userId) {
        ...
        for (const item of purchasedItems) {
            const result = await this.repo.upsertInventoryItem(list.MaNhom, item.TenThucPham, item.SoLuong, item.DonVi, userId);
            if (result === 'added')
                addedCount++;
            else
                mergedCount++;
     ...
     */
    async mergeAllDuplicates(listId, userId) {
        await this.checkListOwnership(listId, userId);
